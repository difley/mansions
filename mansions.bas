...
SUB INITIALIZE
    ...
    CLS
    PRINT "LOADING..."
    ABORT% = LOADFILE("images.0", 12008, VARSEG(IMAGE&(0)), 0, 3)
    IF ABORT% = 1 THEN EXIT SUB
    ABORT% = LOADFILE("pal.0", 212, VARSEG(COLORS&(0)), 0, 3)
    IF ABORT% = 1 THEN EXIT SUB
    ABORT% = LOADFILE("menu.0", 64008, &HA000, 0, 3)
    IF ABORT% = 1 THEN EXIT SUB
    ABORT% = LOADFILE("loadmenu.0", 30008, &HA000, 0, 2)
    IF ABORT% = 1 THEN EXIT SUB
    ABORT% = LOADFILE("savemenu.0", 30008, &HA000, 0, 2)
    IF ABORT% = 1 THEN EXIT SUB
    ABORT% = LOADFILE("message.1", 15008, &HA000, 0, 2)
    IF ABORT% = 1 THEN EXIT SUB
    ABORT% = LOADFILE("message.2", 15008, &HA000, 0, 2)
    IF ABORT% = 1 THEN EXIT SUB
    ABORT% = LOADFILE("message.3", 15008, &HA000, 0, 2)
    IF ABORT% = 1 THEN EXIT SUB
